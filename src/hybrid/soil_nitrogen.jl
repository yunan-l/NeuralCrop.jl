function soil_nitrogen!(nn_model,
    ...
    soil.litn, soil.decom_litn = hybrid_litn(nn_model.lit, soil.litn, ps.ps_litd, st.st_litd, input, soil.respose_litn)
    ...
    soil.fastn, soil.decom_fastn = hybrid_soiln(nn_model.soil, soil.fastn, ps.ps_soild, st.st_soild, input, soil.respose_fastn, soil.n_shift_fast, sum(soil.decom_litn, dims = 1))
    soil.slown, soil.decom_slown = hybrid_soiln(nn_model.soil, soil.slown, ps.ps_soild, st.st_soild, input, soil.respose_slown, soil.n_shift_slow, sum(soil.decom_litn, dims = 1))
